...
1. **ALWAYS update both files:**
   ...
   - `package-lock.json` - MUST be regenerated by running `npm install`
...
3. **Why this matters:**
   - Vercel uses `package-lock.json` for deterministic installs
   - If lockfile is missing the new dependency, the build will FAIL
   - This has caused production outages - do NOT skip this step

**If you cannot run npm install locally**, create a follow-up task or notify the team that the lockfile needs updating.

---

## Cross-System Integration Checklist

When writing frontend code that calls backend APIs:

### Before Writing Fetch Calls:
- [ ] **Verify endpoint exists:** Check `lib/agent/server.py` for the `@app.get` or `@app.post` decorator
- [ ] **Check CORS configuration:** Verify `dream-team-strug.vercel.app` is in the `allow_origins` list in `server.py`
- [ ] **Inspect environment variables:** Check how `NEXT_PUBLIC_API_URL` is set - handle trailing slashes defensively

### URL Construction:
```typescript
// ALWAYS strip trailing slashes to prevent "//endpoint" URLs
const baseUrl = (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/+$/, '')
```

### Environment Variables:
- [ ] **Verify env vars are set in Vercel:** Not just in `.env.local`
- [ ] **Check for placeholder values:** `"your-api-key-here"` is NOT a valid key
- [ ] **NEXT_PUBLIC_ prefix required:** For client-side access in Next.js

### API Authentication:
- [ ] **Include API key in headers:** `'X-API-Key': process.env.NEXT_PUBLIC_API_KEY`
- [ ] **Handle auth errors gracefully:** Show user-friendly message for 401/403
...
- [ ] **Did I update package-lock.json if I added dependencies?**
- [ ] **Did I verify CORS is configured for my API calls?**
- [ ] **Did I handle environment variables defensively?**
